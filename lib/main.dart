...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _cargarEmpresas() async {
    try {
      // Cargar empresas desde la colección empresas
      final empresasSnapshot = await FirebaseFirestore.instance
          .collection('empresas')
          .get();

      final empresas = empresasSnapshot.docs.map((doc) {
        final data = doc.data();
        return Empresa(
          id: doc.id,
          nombre: data['nombre'] ?? '',
      ...
      empresas.sort((a, b) => a.nombre.compareTo(b.nombre));
  ...
  Future<void> _agregarEmpresa(Empresa empresa) async {
    try {
      LoggerService().info('Actualizando lista local de empresas...');
      ...
      LoggerService().info('Lista local actualizada con la nueva empresa');
    } catch (e) {
      LoggerService().error('Error al actualizar lista de empresas', e);
      rethrow;
    }
  }
  ...
  Future<void> _verificarAcceso() async {
    ...
    try {
      // Primero verificar si existe el usuario en la colección usuarios
